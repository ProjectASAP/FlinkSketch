...
/** Example showing memory overhead of exact computation without FlinkSketch. */
public class BeforeFlinkSketch {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // Apply exact quantile computation - stores ALL data points in memory
    // This approach has high memory overhead for large streams
    combinedStream
        .keyBy(item -> item.key)
        .window(TumblingEventTimeWindows.of(Time.seconds(300)))
        .aggregate(new ExactQuantiles(null, new HashMap<>()));

    env.execute("Before FlinkSketch Demo");
  }
}
